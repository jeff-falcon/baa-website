export function getContrastYIQFromColor(color: string) {
  ...
  if (color.startsWith('hsl(')) {
    const hsl = color.replace(/[^\d,]/g, '').split(',').map(Number)
    if (hsl.length !== 3) throw Error('not a valid hsl string');
    const rgb = hslToRgb(hsl[0], hsl[1], hsl[2]);
    console.log({ color, hsl, rgb })
    return getContrastYIQFromCssRGB(`rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`);
  }
  console.log({ color })
